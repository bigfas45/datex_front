...
const UpdateUser = ({match}) => {
  ...
  const init = (itemId) => {
    getUser(itemId,token).then(data => {
        if(data.error){
              setValues({...values, error: data.error})
        }else{
          // populate the state
          setValues({...values, name: data.name, email: data.email, company: data.company,telephone: data.telephone, role: data.role, group: data.group, code: data.code})
          // load security
...
useEffect(() => {
    ...
    const clickSubmit = event => {
        ...
        updateUser(match.params.itemId, user._id,token, {name, email , company, telephone, role, group, code, password})
        .then(data => {
            if (data.error) {
